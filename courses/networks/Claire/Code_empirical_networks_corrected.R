...
#####################################################
# Let's focus on a small food-web first:
i = 6
J = C_mat[[i]]
S = nrow(J)
...
help(mvrnorm) # useful function to generate bivariate normal distribution (for example: with mean of positive elements, mean of negative elements and covariance matrix between negative and positive elements)
BD=mvrnorm(n = 100, c(mean(pairs[,1]),mean(pairs[,2])), Sigma=cov(pairs))
######################################################################
# An example of randimization test
