...
####-----  THE ROSENZWEIG AND MACARTHUR MODEL (RMA) --------------------------
...
####----- Isoclines and phase plane -------------------------------------------
{
  ...
  ####----- Example of stable point -------------------------------------
  {
    ...
    pdf(here::here("img","3_isoclines_rma_3.pdf"), width = 6,height = 5)
    {
    ...
    pdf(here::here("img","4_dynamics_stable.pdf"), width = 5,height = 3)
    {
  ...
  ####----- Example of oscillatory dynamics  -------------------------------------
  {
    ...
    pdf(here::here("img","5_isoclines_rma_V1_oscillatory.pdf"), width = 6,height = 5)
    {
      ####----- calculate the isoclines with the functions
      nullclineP = nullclineP_Hstar(r0,K,a,h,Ps)
      nullclineH = nullclineH_Pstar(eps,a,h,m)
    ...
    pdf(here::here("img","6_dynamics_oscillatory.pdf"), width = 5,height = 3)
    {
      dyn = data.frame(ode(y = yini, times = simtime, func = rma_model, parms = pars, method = "rk4"))
      plot(dyn[, "time"], dyn[, "P"], col="chartreuse3",type = "l", ylim=range(dyn[,c("P","H")]), ylab = "Density", xlab = "Time",lwd=2)
      lines(dyn[, "time"], dyn[, "H"], type = "l", col = "darkorange3",lwd=2)
...

{
  ...
  pdf(here::here("img","7_euler_Logistic.pdf"), width = 5,height = 5.5)
...

{
  ...
  pdf(here::here("img","8_euler_LV.pdf"), width = 5,height = 5.5)
...
####-----  Times series for the RMA model  --------------------------

{
  ...
  ###---- Plot trajectories
  pdf(here::here("img","9_rma_dynamics.pdf"), width = 7,height = 4)
  {
...
####----- Bifurcation diagram - Stability criteria ----------------------------
{
  ...
  ####----- Record stability criteria for the steady state found by the stode function
  lambdamax = NULL 
  for(i in 1: length(Ks)){
    pars["K"] = Ks[i]
